// Sharp Anchor — API-Sports MLB (Pinnacle / Circa feeds).
// Same parser shape as the NBA adapter; only the BASE host + season
// format differ. MLB seasons key on the calendar year.

import type { ApiSportsSnapshot } from './api-sports'

const BASE = process.env.API_SPORTS_MLB_BASE_URL ?? 'https://v1.baseball.api-sports.io'

export async function fetchMlbSharpSnapshots(opts: {
  signal?: AbortSignal
  ...
  try {
    const res = await fetch(`${BASE}/games?league=1&season=${new Date().getUTCFullYear()}`, {
...
function mapStatus(short: string | undefined): 'scheduled' | 'live' | 'final' {
  ...
  if (short === 'FT' || short === 'AOT' || short === 'FN') return 'final'
