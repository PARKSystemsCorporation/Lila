// Prediction Market — ProphetX (peer-to-peer).
// Returns implied probabilities per side and the resulting overround.
// Overround percent = (impliedHome + impliedAway - 1) * 100; this is the
// direct input for the operator's first 1–10 scale.

const BASE = process.env.PROPHET_X_BASE_URL ?? 'https://api.prophetx.co/v1'

export type ProphetXSnapshot = {
  away_team:       { city: string; name: string }
  home_team:       { city: string; name: string }
  implied_prob:    { home: number; away: number }     // 0..1
  overround_pct:   number                              // percent above 100
  observed_at:     string
}

export async function fetchNbaPredictionSnapshots(opts: {
  signal?: AbortSignal
  ...
  try {
    const res = await fetch(`${BASE}/markets/nba`, {
...
function parseSnapshots(body: unknown): ProphetXSnapshot[] {
  ...
  for (const raw of markets) {
    ...
    // Treat probabilities outside (0, 1] as garbage and drop them.
