// Retail Sensor — ParlayAPI NFL (DraftKings / FanDuel / MGM).
// Same shape as the NBA adapter (lib/sports/sources/parlay.ts); only
// the path segment differs. Returns null when not configured.

import type { ParlaySnapshot } from './parlay'

const BASE = process.env.PARLAY_API_BASE_URL ?? 'https://api.parlay.io/v1'

export async function fetchNflRetailSnapshots(opts: {
  signal?: AbortSignal
  ...
  try {
    const res = await fetch(`${BASE}/odds/nfl`, {
