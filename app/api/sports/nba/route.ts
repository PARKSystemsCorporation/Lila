// Live NBA portal payload — replaces the hardcoded GAMES/SIGNALS arrays
// in app/theyield/sports/nba/page.tsx. Gated by the same lila_viewer
// cookie as the rest of /theyield.
...
export async function GET() {
  ...
  try {
    ...
    const games: GamePayload[] = rows.rows.map((r) => ({
      ...
      signals: {
        // We surface the lead team's signals for the slot machine — the
        // operator's framework treats the underdog as the binary-zero side.
